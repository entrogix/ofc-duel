# OFCデュエル — オープンフェイス・チャイニーズポーカー

ボードゲームブログの [OFCターボ](http://boardgameblog.net/2022/06/02/ocpt/) をベースにしたスマホアプリ。
...
```bash
cd packages/shared
npx tsx --test test/*.test.ts   # 役判定・採点・エンジン・AI・レート 23テスト
...
- 6枚 → 5枚 → 5枚 の3回に分けて配置（各ストリート1枚捨て）、そのたび全員同時公開
- フロントQQ以上でファンタジーランド（次ハンド14枚配り・13枚配置）
- 初期チップ50点。上限6ハンドか誰かが破産したら終了
